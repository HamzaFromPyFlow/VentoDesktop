...
export default function RemoveTeamMember({
  ...
  if (removed === REMOVAL_TYPE.KEEP_CONTENT) {
    return (
      <div className={styles.successContainer}>
        <p>
          {email} has been removed from your team!
        </p>
  ...
  if (removed === REMOVAL_TYPE.MOVE_CONTENT) {
    return (
      <div className={styles.successContainer}>
        <p>
          {email} has been removed from your team and a folder of {email ? `${email}'s` : 'their'} content has been added to your recordings.
        </p>
        <button className={styles.gotItBtn} onClick={onClose}>
          Got it
        </button>
  ...
  return (
    <div className={styles.successContainer}>
      <p className={styles.UserText}>
        When you select &#34;Remove User&#34;, {email} will no longer have access to your team and workspaces.
        {isPremium && billingInfo &&
          <>
            {` We'll refund a prorated amount of $${billingInfo.pcpu} for the remainder of the ${billingInfo.planFrequency}.`}
            <br />
          </>
        }
        {!isDeclined && checkRecording &&
          <b>Please choose your removal type:</b>
        }
      </p>
      {!isDeclined && (
        <>
          {loadingRecordings ? (
            ...
            <div className={styles.removeUserRadioGroup}>
              <label className={styles.removeUserRadioLabel}>
                ...
                />
                Remove user from team and workspaces and move user content to admin
              </label>
              <label className={styles.removeUserRadioLabel}>
                <input
                  type="radio"
                ...
                />
                Remove user from team and workspaces, allow user to keep all of their content
              </label>
            </div>
          ) : null}
        </>
